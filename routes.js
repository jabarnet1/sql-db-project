...
const getTeamRoster = async function(req, res) {
  ...
  connection.query(
    `SELECT DISTINCT g.player_id, p.firstname, p.lastname, p.primaryPosition, 
     ...
     ORDER BY p.primaryPosition, points DESC NULLS LAST`,
    [team_id, season],
    (err, data) => {
      if (err) {
        console.error(err);
...
const getExpectedVsActualGoals = async function(req, res) {
    ...
    if (team_id) {
      query += ` AND ti.team_id = $${paramIndex}`;
      params.push(team_id);
      paramIndex++;
    }
    
    if (team_name) {
      query += ` AND (LOWER(ti.teamname) LIKE LOWER($${paramIndex}) OR LOWER(ti.abbreviation) LIKE LOWER($${paramIndex}))`;
      params.push(`%${team_name}%`);
      paramIndex++;
    }
    
    query += `
      GROUP BY ss.team, ti.teamname, ss.season
...
const getTeamStandings = async function(req, res) {
  ...
  const getTeamPerformanceMetrics = async function(req, res) {
    ...
    const query = `
      ...
      WHERE ts.season = $1
      AND ts.team_id = $2  -- Filter by team_id
    ...
    (err, profileData) => {
      ...
      connection.query(
        `SELECT 
           ...
           ROUND((i_f_goals * 1.0 / NULLIF(i_f_shotsOnGoal, 0)) * 100, 1) AS shooting_percentage,
   ...
   (err, headToHeadData) => {
     ...
     connection.query(
       ...
       (err, gamesData) => {
         ...
         if (!headToHeadData.rows || headToHeadData.rows.length === 0) {
          return res.status(404).json({ error: 'No head-to-head data found for the selected teams' });
        }
